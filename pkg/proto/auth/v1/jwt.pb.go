...
type IssueTokenRequest struct {
	...
	// Дополнительные claims для включения в токен
	...
	// Идентификатор устройства (опционально, для отслеживания)
	DeviceId string `protobuf:"bytes,5,opt,name=device_id,json=deviceId,proto3" json:"device_id,omitempty"`
	// Project ID (UUID) - идентификатор проекта (опционально, если не указан - используется default_project_id)
	// Если не указан в запросе, может быть передан через gRPC metadata "project-id"
...
// TokenClaims - стандартные claims JWT токена
type TokenClaims struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// User ID (sub claim)
	UserId string `protobuf:"bytes,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	// Issued at (iat claim, Unix timestamp)
	IssuedAt int64 `protobuf:"varint,2,opt,name=issued_at,json=issuedAt,proto3" json:"issued_at,omitempty"`
	// Expires at (exp claim, Unix timestamp)
	...
	// Дополнительные кастомные claims
	CustomClaims map[string]string `protobuf:"bytes,7,rep,name=custom_claims,json=customClaims,proto3" json:"custom_claims,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
...
func (x *TokenClaims) GetCustomClaims() map[string]string {
	if x != nil {
...
const file_jwt_proto_rawDesc = "" +
	...
	"\aresults\x18\x01 \x03(\v2\x1e.auth.v1.ValidateTokenResponseR\aresults\"\xf1\x02\n" +
	...
	"\x06jwt_id\x18\x06 \x01(\tR\x05jwtId\x12K\n" +
	"\rcustom_claims\x18\a \x03(\v2&.auth.v1.TokenClaims.CustomClaimsEntryR\fcustomClaims\x12\x1d\n" +
	"\n" +
	"token_type\x18\b \x01(\tR\ttokenType\x12\x15\n" +
	...
	"\vListAPIKeys\x12\x1b.auth.v1.ListAPIKeysRequest\x1a\x1c.auth.v1.ListAPIKeysResponseB5Z3github.com/octawire/auth-service/api/auth/v1;authv1b\x06proto3"
...
var file_jwt_proto_msgTypes = make([]protoimpl.MessageInfo, 36)
var file_jwt_proto_goTypes = []any{
	...
	nil,                              // 33: auth.v1.TokenClaims.CustomClaimsEntry
	nil,                              // 34: auth.v1.HealthCheckResponse.DetailsEntry
	nil,                              // 35: auth.v1.IssueServiceTokenRequest.ClaimsEntry
	nil,                              // 36: auth.v1.ValidateAPIKeyResponse.MetadataEntry
}
var file_jwt_proto_depIdxs = []int32{
	...
	33, // 6: auth.v1.TokenClaims.custom_claims:type_name -> auth.v1.TokenClaims.CustomClaimsEntry
	18, // 7: auth.v1.GetPublicKeyResponse.active_keys:type_name -> auth.v1.PublicKeyInfo
	34, // 8: auth.v1.HealthCheckResponse.details:type_name -> auth.v1.HealthCheckResponse.DetailsEntry
	35, // 9: auth.v1.IssueServiceTokenRequest.claims:type_name -> auth.v1.IssueServiceTokenRequest.ClaimsEntry
	0,  // 10: auth.v1.ValidateAPIKeyResponse.error_code:type_name -> auth.v1.ErrorCode
	36, // 11: auth.v1.ValidateAPIKeyResponse.metadata:type_name -> auth.v1.ValidateAPIKeyResponse.MetadataEntry
	30, // 12: auth.v1.ListAPIKeysResponse.keys:type_name -> auth.v1.APIKeyInfo
	1,  // 13: auth.v1.JWTService.IssueToken:input_type -> auth.v1.IssueTokenRequest
	3,  // 14: auth.v1.JWTService.RefreshToken:input_type -> auth.v1.RefreshTokenRequest
	21, // 15: auth.v1.JWTService.IssueServiceToken:input_type -> auth.v1.IssueServiceTokenRequest
	5,  // 16: auth.v1.JWTService.ValidateToken:input_type -> auth.v1.ValidateTokenRequest
	7,  // 17: auth.v1.JWTService.ParseToken:input_type -> auth.v1.ParseTokenRequest
	9,  // 18: auth.v1.JWTService.ExtractClaims:input_type -> auth.v1.ExtractClaimsRequest
	11, // 19: auth.v1.JWTService.RevokeToken:input_type -> auth.v1.RevokeTokenRequest
	13, // 20: auth.v1.JWTService.ValidateBatch:input_type -> auth.v1.ValidateBatchRequest
	16, // 21: auth.v1.JWTService.GetPublicKey:input_type -> auth.v1.GetPublicKeyRequest
	19, // 22: auth.v1.JWTService.HealthCheck:input_type -> auth.v1.HealthCheckRequest
	22, // 23: auth.v1.APIKeyService.CreateAPIKey:input_type -> auth.v1.CreateAPIKeyRequest
	24, // 24: auth.v1.APIKeyService.ValidateAPIKey:input_type -> auth.v1.ValidateAPIKeyRequest
	26, // 25: auth.v1.APIKeyService.RevokeAPIKey:input_type -> auth.v1.RevokeAPIKeyRequest
	28, // 26: auth.v1.APIKeyService.ListAPIKeys:input_type -> auth.v1.ListAPIKeysRequest
	2,  // 27: auth.v1.JWTService.IssueToken:output_type -> auth.v1.IssueTokenResponse
	4,  // 28: auth.v1.JWTService.RefreshToken:output_type -> auth.v1.RefreshTokenResponse
	2,  // 29: auth.v1.JWTService.IssueServiceToken:output_type -> auth.v1.IssueTokenResponse
	6,  // 30: auth.v1.JWTService.ValidateToken:output_type -> auth.v1.ValidateTokenResponse
	8,  // 31: auth.v1.JWTService.ParseToken:output_type -> auth.v1.ParseTokenResponse
	10, // 32: auth.v1.JWTService.ExtractClaims:output_type -> auth.v1.ExtractClaimsResponse
	12, // 33: auth.v1.JWTService.RevokeToken:output_type -> auth.v1.RevokeTokenResponse
	14, // 34: auth.v1.JWTService.ValidateBatch:output_type -> auth.v1.ValidateBatchResponse
	17, // 35: auth.v1.JWTService.GetPublicKey:output_type -> auth.v1.GetPublicKeyResponse
	20, // 36: auth.v1.JWTService.HealthCheck:output_type -> auth.v1.HealthCheckResponse
	23, // 37: auth.v1.APIKeyService.CreateAPIKey:output_type -> auth.v1.CreateAPIKeyResponse
	25, // 38: auth.v1.APIKeyService.ValidateAPIKey:output_type -> auth.v1.ValidateAPIKeyResponse
	27, // 39: auth.v1.APIKeyService.RevokeAPIKey:output_type -> auth.v1.RevokeAPIKeyResponse
	29, // 40: auth.v1.APIKeyService.ListAPIKeys:output_type -> auth.v1.ListAPIKeysResponse
